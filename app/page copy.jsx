...
function WaveCanvas({ active }) {
  ...
  const run = useCallback(() => {
    ...
    let t = 0;
    const layers = [
      { a: 0.45, s: 0.012, o: 0, col: "rgba(255,107,107,0.18)" },
      { a: 0.35, s: 0.018, o: 2,  col: "rgba(255,142,83,0.12)" },
      { a: 0.28, s: 0.024, o: 4,  col: "rgba(0,201,219,0.10)" },
      { a: 0.18, s: 0.032, o: 6,  col: "rgba(255,217,61,0.08)" },
    ];
    const draw = () => {
      ...
      layers.forEach(l => {
        ...
        ctx.moveTo(0, H);
        for (let x = 0; x <= W; x += 4) {
          const y = H / 2 + Math.sin((x * l.s) + t + l.o) * H * l.a
                  + Math.sin((x * l.s * 1.7) + t * 1.3 + l.o) * H * 0.08;
          ctx.lineTo(x, y);
        }
  ...
  useEffect(() => {
    if (active) { run(); }
    else { cancelAnimationFrame(anim.current); const c = ref.current; if (c) c.getContext("2d").clearRect(0,0,c.width,c.height); }
    return () => cancelAnimationFrame(anim.current);
  }, [active, run]);

  return <canvas ref={ref} style={{ position:"fixed", inset:0, width:"100%", height:"100%", pointerEvents:"none", zIndex:99, opacity: active ? 1 : 0, transition:"opacity 0.4s" }} />;
}
export default function App() {
  const [tab, setTab] = useState("h");
const [wave, setWave] = useState(false);
const changeTab = (id) => { setWave(true); setTimeout(() => { setTab(id); setWave(false); }, 700); };
  return (
    <>
      <style>{`
