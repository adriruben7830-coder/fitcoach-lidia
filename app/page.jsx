  ...
  return (
    <div style={{ textAlign: "center" }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <text x={size/2} y={size/2+5} textAnchor="middle" fill="#fff" fontSize="13" fontWeight="700">{value}</text>
      </svg>
  ...
  return (
    <div className="pg">
      <div className="hero-card">
        <div className="hero-eyebrow">🌺 Tu progreso</div>
        <div className="hero-name">{P.name}</div>
        <div className="hero-sub">Objetivo: 70kg masa muscular · mínima grasa 💪</div>
      ...
      <div className="sg" style={{ marginBottom:14 }}>
        <div className="sb"><div className="v" style={{ color:"#ff6b6b" }}>{P.weight}</div><div className="l">Actual kg</div></div>
      ...
      <div className="c" style={{ marginBottom:14 }}>
        ...
        <div className="bar">
          <div className="fill" style={{ width:`${((P.weight-55)/(P.targetWeight-55))*100}%` }} />
        </div>
        <p style={{ fontSize:11, color:"#7a8ba8", marginTop:8, textAlign:"center" }}>Ganancia limpia · más músculo, menos grasa</p>
      </div>

      <div className="st">📊 Métricas</div>
      <div className="metric-grid" style={{ marginBottom:14 }}>
        {[
          { v:P.bmr, l:"BMR", c:"#FFD93D" },
          { v:P.bodyFat+"%", l:"Grasa", c:"#ff6b6b" },
          { v:P.water+"%", l:"Agua", c:"#38bdf8" },
          { v:P.skeletalMuscle+"%", l:"M.Esq", c:"#4ade80" },
          { v:P.visceralFat, l:"G.Vis", c:"#c084fc" },
          { v:P.bmi, l:"IMC", c:"#fb923c" },
        ].map(m => (
      ...
      <div className="tip-box">
        ...
        <p className="tip-text">Para ganar músculo necesitas comer en <strong style={{ color:"#fff" }}>superávit calórico controlado</strong> (~2400kcal) con alta proteína (145g). No tengas miedo a comer más — tu cuerpo necesita ese combustible. 💪</p>
      </div>
    </div>
  );
}
...
function Train() {
  ...
  return (
    <div className="pg">
      <div className="st">🏋️‍♀️ Entreno</div>
      ...
      <div className="c cg" style={{ marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22, letterSpacing:1 }}>{d.title}</div>
      ...
      <div className="c" style={{ marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          <span style={{ fontSize:13, fontWeight:700 }}>⏱️ Timer descanso</span>
          <span style={{ fontSize:20, fontWeight:700, color: timeLeft===0 ? "#4ade80" : running ? "#ff6b6b" : "#f0f4ff", fontFamily:"'Bebas Neue',sans-serif", letterSpacing:1 }}>{timeLeft}s</span>
        </div>
        <div style={{ height:6, background:"rgba(255,255,255,.06)", borderRadius:99, overflow:"hidden", marginBottom:12 }}>
          <div style={{ height:"100%", borderRadius:99, background: timeLeft===0 ? "#4ade80" : "linear-gradient(90deg,#ff6b6b,#ff8e53)", width:`${(timeLeft/90)*100}%`, transition:"width 1s linear" }} />
        </div>
        <button onClick={startTimer} style={{ width:"100%", padding:"10px 0", borderRadius:14, border:"none", background: running ? "rgba(239,68,68,.15)" : "rgba(255,107,107,.15)", color: running ? "#f87171" : "#ff6b6b", fontSize:14, fontWeight:700, cursor:"pointer", border: running ? "1px solid rgba(239,68,68,.3)" : "1px solid rgba(255,107,107,.3)" }}>
          {timeLeft===0 ? "✅ ¡Listo! Reiniciar" : running ? "⏹ Parar" : "▶ Iniciar 90s"}
        </button>
      </div>
...
function Food() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) { const p=JSON.parse(saved); if(p.date===todayKey()) setCheckedMeals(p.items||{}); }
    } catch {}
  }, []);

  const saveChecked = (items) => localStorage.setItem("fc_checked", JSON.stringify({ date:todayKey(), items }));

  const consumed = log.items.reduce((a,i) => ({ cal:a.cal+(i.cal||0), prot:a.prot+(i.prot||0), carbs:a.carbs+(i.carbs||0), fat:a.fat+(i.fat||0) }), { cal:0, prot:0, carbs:0, fat:0 });
  ...
  const toggleMealCheck = (dayIdx, mealIdx) => {
    const key=`${dayIdx}-${mealIdx}`, meal=getMeal(dayIdx,mealIdx);
    const newChecked={ ...checkedMeals };
    if(checkedMeals[key]) {
      const logIdx=log.items.findIndex(i=>i.planKey===key);
      if(logIdx!==-1) { const updated={...log,items:log.items.filter((_,i)=>i!==logIdx)}; setLog(updated); saveLog(updated); }
      ...
      newChecked[key]=true;
      const totalMeals=DEFAULT_MEALS[dayIdx].meals.length;
      const checkedCount=Object.keys(newChecked).filter(k=>k.startsWith(`${dayIdx}-`)).length;
      if(checkedCount===totalMeals) { setShowConfetti(true); setTimeout(()=>setShowConfetti(false),3500); }
    }
    setCheckedMeals(newChecked); saveChecked(newChecked);
  };
  ...
  const confirmAdd=()=>{ if(result){ addToLog({name:result.alimento,cal:result.calorias||0,prot:result.proteinas||0,carbs:result.carbohidratos||0,fat:result.grasas||0}); resetPhoto(); } };
  ...
  return (
    <div className="pg">
        ...
        <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:999, overflow:"hidden" }}>
          {Array.from({length:60}).map((_,i)=>(
            <div key={i} style={{
              ...
              background:["#ff6b6b","#4ade80","#FFD93D","#00C9DB","#c084fc","#fb923c"][Math.floor(Math.random()*6)],
      ...
      <div className="c cg">
        ...
        <ProgressBar value={consumed.prot} max={MACROS.protein} color="#ff6b6b" label="Proteína (g)"/>
        ...
        <div className="c" style={{ padding:14 }}>
          ...
          {log.items.map((item,i)=>(
            <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:i<log.items.length-1?"1px solid rgba(255,255,255,.04)":"none" }}>
              ...
              <button onClick={()=>{ if(item.planKey){ const n={...checkedMeals}; delete n[item.planKey]; setCheckedMeals(n); saveChecked(n); } removeFromLog(i); }} style={{ background:"none", border:"none", cursor:"pointer", padding:4 }}>
      ...
      <input type="file" ref={ref} accept="image/*" capture="environment" style={{ display:"none" }} onChange={e=>{ const f=e.target.files?.[0]; if(!f) return; setPreview(URL.createObjectURL(f)); analyze(f); }}/>

      {!preview&&!busy&&!result&&!error && (
        <div className="up" onClick={()=>ref.current?.click()}>
          <Camera size={36} color="#ff6b6b" style={{ margin:"0 auto 12px" }}/>
      ...
      {error && <div className="c" style={{ borderColor:"rgba(239,68,68,.25)", background:"rgba(239,68,68,.06)" }}><p style={{ fontSize:13, color:"#fca5a5", marginBottom:6 }}>⚠️ {error}</p><button onClick={resetPhoto} style={{ width:"100%", padding:12, borderRadius:12, border:"1px solid rgba(255,255,255,.08)", background:"rgba(255,255,255,.04)", color:"#f0f4ff", fontSize:14, fontWeight:600, cursor:"pointer" }}>Reintentar</button></div>}

      {result && (
        <div className="c cg">
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <h3 style={{ fontSize:16, fontWeight:700 }}>{result.alimento}</h3>
            <div style={{ background:"linear-gradient(135deg,#ff6b6b,#ff8e53)", borderRadius:12, padding:"4px 12px", fontWeight:700, fontSize:14 }}>{result.puntuacion}/10</div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:6, marginBottom:12 }}>
            {[{l:"Cal",v:result.calorias,c:"#FFD93D"},{l:"Prot",v:result.proteinas,c:"#ff6b6b"},{l:"Carbs",v:result.carbohidratos,c:"#c084fc"},{l:"Grasa",v:result.grasas,c:"#00C9DB"}].map(m=>(
          ...
          <div style={{ display:"flex", gap:8, marginTop:14 }}>
            ...
            <button onClick={resetPhoto} style={{ padding:12, borderRadius:14, border:"1px solid rgba(255,255,255,.08)", background:"rgba(255,255,255,.04)", color:"#7a8ba8", fontSize:14, cursor:"pointer" }}>
              <X size={16}/>
            </button>
          </div>
        </div>
      ...
      <div className="st" style={{ marginTop:20 }}>🥗 Plan Comidas</div>
      ...
      <div className="c" style={{ background:"rgba(255,217,61,.04)", borderColor:"rgba(255,217,61,.1)", padding:14 }}>
        {dayMeals.map((m,i)=>{
          const checkKey=`${mealDay}-${i}`, isChecked=!!checkedMeals[checkKey];
          return (
            <div key={i} className="mc" style={{ opacity:isChecked?0.6:1 }}>
              {editing?.dayIdx===mealDay&&editing?.mealIdx===i ? (
                <div>
                  <div style={{ fontSize:13, fontWeight:700, marginBottom:6 }}>{m.time}</div>
                  <textarea value={editText} onChange={e=>setEditText(e.target.value)} placeholder="Escribe lo que vas a comer..."
                    style={{ width:"100%", padding:10, borderRadius:10, border:"1px solid rgba(255,255,255,.08)", background:"rgba(255,255,255,.04)", color:"#f0f4ff", fontSize:13, fontFamily:"'Satoshi',sans-serif", resize:"vertical", minHeight:60, outline:"none" }}/>
                ...
                <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                  <button onClick={()=>toggleMealCheck(mealDay,i)} style={{ width:28, height:28, borderRadius:8, border:isChecked?"none":"2px solid rgba(255,255,255,.15)", background:isChecked?"#4ade80":"transparent", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginTop:2 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex", gap:8, marginTop:6, fontSize:11, color:"#7a8ba8" }}>
                      ...
                      <span><strong style={{ color:"#ff6b6b" }}>{m.prot}g</strong> prot</span>
        ...
        <div style={{ textAlign:"center", marginTop:10, fontSize:12, color:"#7a8ba8" }}>
          Total plan: <strong style={{ color:"#FFD93D" }}>{dayTotal.cal} kcal</strong> · <strong style={{ color:"#ff6b6b" }}>{dayTotal.prot}g prot</strong>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="pg" style={{ paddingBottom:0, height:"calc(100vh - 135px)", display:"flex", flexDirection:"column" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, flexShrink:0 }}>
        <div className="st" style={{ margin:0 }}>🌺 Chat Coach</div>
        <button onClick={clearChat} style={{ background:"none", border:"1px solid rgba(255,255,255,.08)", borderRadius:10, padding:"4px 10px", color:"#7a8ba8", fontSize:12, cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
...
function WaveTransition({ active }) {
  const ref = useRef(null);
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      const layers = [
        { offset: 0,   amp: 40, speed: 1.2, color: "rgba(255,107,107,0.35)" },
        { offset: 0.3, amp: 30, speed: 1.8, color: "rgba(255,142,83,0.25)" },
        { offset: 0.6, amp: 50, speed: 0.9, color: "rgba(0,201,219,0.20)" },
        { offset: 0.9, amp: 25, speed: 2.1, color: "rgba(5,12,23,0.95)" },
      ];

      layers.forEach((l, idx) => {
        const layerY = waveY + (idx * 18);
        ctx.beginPath();
        ctx.moveTo(0, H);
        for (let x = 0; x <= W; x += 3) {
          const wave1 = Math.sin((x / W) * Math.PI * 3 + progress * l.speed * 8 + l.offset) * l.amp;
          const wave2 = Math.sin((x / W) * Math.PI * 5 + progress * l.speed * 6) * (l.amp * 0.4);
      ...
      if (progress > 0.1 && progress < 0.9) {
        for (let i = 0; i < 8; i++) {
          const px = (W / 8) * i + Math.sin(progress * 10 + i) * 20;
          const py = waveY + Math.sin((px / W) * Math.PI * 3 + progress * 8) * 40;
          ctx.beginPath();
...
export default function App() {
  ...
  const changeTab = (id) => {
    if (id === tab) return;
    setWave(true);
    setTimeout(() => { setTab(id); setWave(false); }, 1400);
  };

  return (
    <>
      <style>{`
@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Satoshi:ital,wght@0,300;0,400;0,500;0,700;0,900;1,300&display=swap');
*{box-sizing:border-box;margin:0;padding:0}
body{background:#050c17;color:#f0f4ff;font-family:'Satoshi',sans-serif;-webkit-tap-highlight-color:transparent}
...
body::after{content:'';position:fixed;inset:0;background:radial-gradient(ellipse 600px 400px at 80% -10%,rgba(255,107,100,.07) 0%,transparent 70%),radial-gradient(ellipse 500px 500px at -10% 60%,rgba(80,100,255,.06) 0%,transparent 70%),radial-gradient(ellipse 400px 300px at 60% 110%,rgba(0,201,219,.05) 0%,transparent 70%);pointer-events:none;z-index:0}
.app{max-width:430px;margin:0 auto;min-height:100vh;min-height:100dvh;background:transparent;padding-bottom:85px;overflow-x:hidden;position:relative;z-index:1}
.hd{padding:14px 20px 12px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:50;background:rgba(5,12,23,.92);backdrop-filter:blur(24px);border-bottom:1px solid rgba(255,255,255,.06)}
.hd-logo{display:flex;align-items:center;gap:10px}
.hd-mark{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,#ff6b6b,#ff8e53);display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 4px 16px rgba(255,107,107,.35)}
.hd-word{font-family:'Bebas Neue',sans-serif;font-size:26px;letter-spacing:2px;color:#f0f4ff}
.hd-word span{color:#ff6b6b}
.pulse-badge{display:flex;align-items:center;gap:5px;background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.2);border-radius:20px;padding:4px 10px}
.pulse-dot{width:7px;height:7px;border-radius:50%;background:#4ade80;box-shadow:0 0 8px rgba(74,222,128,.6);animation:pulse-anim 2s ease infinite}
.pulse-txt{font-size:10px;color:#4ade80;font-weight:700;letter-spacing:.5px}
@keyframes pulse-anim{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.6;transform:scale(.85)}}
.pg{padding:8px 20px 20px;animation:fu .35s ease;position:relative;z-index:1}
...
@keyframes wv{0%{transform:rotate(0)}25%{transform:rotate(14deg)}50%{transform:rotate(0)}75%{transform:rotate(-8deg)}100%{transform:rotate(0)}}
...
.c{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:22px;padding:18px;margin-bottom:14px;backdrop-filter:blur(10px)}
.cg{background:linear-gradient(135deg,rgba(255,107,107,.07),rgba(0,201,219,.04));border-color:rgba(255,107,107,.15)}
.co{background:linear-gradient(135deg,rgba(0,201,219,.07),rgba(107,91,255,.04));border-color:rgba(0,201,219,.15)}
.hero-card{background:linear-gradient(135deg,rgba(255,107,107,.1) 0%,rgba(255,142,83,.06) 50%,rgba(0,201,219,.06) 100%);border:1px solid rgba(255,107,107,.2);border-radius:24px;padding:24px;margin-bottom:14px;position:relative;overflow:hidden}
.hero-card::before{content:'';position:absolute;top:-40px;right:-40px;width:160px;height:160px;background:radial-gradient(circle,rgba(255,107,107,.15),transparent 70%);pointer-events:none}
.hero-eyebrow{font-size:10px;font-weight:700;letter-spacing:2px;color:#ff6b6b;text-transform:uppercase;margin-bottom:8px}
.hero-name{font-family:'Bebas Neue',sans-serif;font-size:52px;line-height:1;color:#f0f4ff;letter-spacing:1px}
.hero-sub{font-size:13px;color:#7a8ba8;margin-top:6px;line-height:1.5}
.sg{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px}
.sb{background:rgba(255,255,255,.03);border-radius:18px;padding:16px 10px;text-align:center;border:1px solid rgba(255,255,255,.06);transition:all .2s;cursor:default}
.sb:hover{background:rgba(255,255,255,.06);border-color:rgba(255,255,255,.12);transform:translateY(-2px)}
.sb .v{font-family:'Bebas Neue',sans-serif;font-size:26px;font-weight:400;line-height:1}
.sb .l{font-size:10px;color:#7a8ba8;margin-top:5px;text-transform:uppercase;letter-spacing:.8px}
.bar{height:8px;background:rgba(255,255,255,.06);border-radius:99px;overflow:hidden;position:relative}
.fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#ff6b6b,#ff8e53,#ffd93d);transition:width 1s;position:relative}
.fill::after{content:'';position:absolute;right:0;top:50%;transform:translateY(-50%);width:10px;height:10px;border-radius:50%;background:#fff;box-shadow:0 0 8px rgba(255,180,100,.8);animation:glow-dot 1.5s ease infinite}
@keyframes glow-dot{0%,100%{opacity:1;box-shadow:0 0 8px rgba(255,180,100,.8)}50%{opacity:.6;box-shadow:0 0 4px rgba(255,180,100,.4)}}
.metric-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px}
.metric-item{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.05);border-radius:14px;padding:12px 8px;text-align:center;transition:all .2s}
.metric-item:hover{background:rgba(255,255,255,.05);transform:translateY(-1px)}
.metric-val{font-family:'Bebas Neue',sans-serif;font-size:22px;line-height:1}
.metric-lbl{font-size:9px;color:#7a8ba8;margin-top:3px;text-transform:uppercase;letter-spacing:.8px}
.tip-box{background:rgba(0,201,219,.06);border:1px solid rgba(0,201,219,.15);border-radius:16px;padding:14px 16px;display:flex;gap:12px;align-items:flex-start}
.tip-icon{font-size:20px;flex-shrink:0}
.tip-text{font-size:12px;color:#9db3cc;line-height:1.65}
.ex{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:18px;padding:14px 16px;margin-bottom:10px;cursor:pointer;transition:transform .15s}
.ex:active{transform:scale(.98)}
.dt{padding:10px 16px;border-radius:14px;font-size:13px;font-weight:700;white-space:nowrap;cursor:pointer;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03);color:#7a8ba8;font-family:'Satoshi',sans-serif;transition:all .2s}
.dt.on{background:linear-gradient(135deg,#ff6b6b,#ff8e53);color:#fff;border-color:transparent;box-shadow:0 4px 15px rgba(255,107,107,.3)}
.ms{max-width:85%;padding:12px 16px;border-radius:20px;font-size:14px;line-height:1.6;animation:fu .2s;word-wrap:break-word}
.ms.u{align-self:flex-end;background:linear-gradient(135deg,#ff6b6b,#ff8e53);color:#fff;border-bottom-right-radius:6px}
.ms.b{align-self:flex-start;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-bottom-left-radius:6px}
.ms.e{align-self:flex-start;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-bottom-left-radius:6px;color:#fca5a5;font-size:12px}
.ci{display:flex;gap:8px;padding:12px 0 4px;border-top:1px solid rgba(255,255,255,.05)}
.ci input{flex:1;padding:12px 16px;border-radius:16px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.04);color:#f0f4ff;font-size:14px;outline:none;font-family:'Satoshi',sans-serif}
.ci input::placeholder{color:#7a8ba8}
.ci input:focus{border-color:#ff6b6b}
.sn{width:48px;height:48px;border-radius:16px;border:none;background:linear-gradient(135deg,#ff6b6b,#ff8e53);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.sn:disabled{opacity:.4;cursor:not-allowed}
.bn{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:rgba(5,12,23,.97);backdrop-filter:blur(24px);border-top:1px solid rgba(255,255,255,.06);padding:8px 12px env(safe-area-inset-bottom,24px);display:flex;justify-content:space-around;z-index:100}
.ni{display:flex;flex-direction:column;align-items:center;gap:4px;cursor:pointer;padding:6px 14px;border-radius:14px;background:0;border:0;color:#7a8ba8;font-family:'Satoshi',sans-serif}
.ni.on2{color:#ff6b6b}
.ni span{font-size:10px;font-weight:700}
.st{font-family:'Satoshi',sans-serif;font-size:17px;font-weight:700;margin-bottom:14px;display:flex;align-items:center;gap:8px}
.up{border:2px dashed rgba(255,107,107,.3);border-radius:20px;padding:28px;text-align:center;cursor:pointer;background:rgba(255,107,107,.04)}
.td span{display:inline-block;width:7px;height:7px;border-radius:50%;background:#7a8ba8;animation:pu 1.2s infinite;margin:0 2px}
.td span:nth-child(2){animation-delay:.2s}
.td span:nth-child(3){animation-delay:.4s}
.tg{display:inline-block;padding:4px 10px;border-radius:10px;font-size:11px;font-weight:600;margin:3px 3px 3px 0}
.tp{background:rgba(255,107,107,.12);color:#ff6b6b;border:1px solid rgba(255,107,107,.18)}
.tb{background:rgba(0,201,219,.12);color:#00C9DB;border:1px solid rgba(0,201,219,.18)}
.wh{display:inline-block;animation:wv 1.8s ease infinite;transform-origin:70% 70%}
.flt{animation:fl 3s ease infinite}
.mc{background:rgba(255,255,255,.02);border-radius:14px;padding:12px 14px;margin-bottom:8px;border:1px solid rgba(255,255,255,.04)}
.dc{padding:8px 14px;border-radius:12px;font-size:12px;font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03);color:#7a8ba8;font-family:'Satoshi',sans-serif;white-space:nowrap;transition:all .2s}
.dc.on3{background:#FFD93D;color:#1a1a2e;border-color:transparent}
.spin{animation:sp 1s linear infinite}
@keyframes sp{from{transform:rotate(0)}to{transform:rotate(360deg)}}
      `}</style>
      <WaveTransition active={wave} />
      <div className="app">
        <div className="hd">
          <div className="hd-logo">
            <div className="hd-mark">⚡</div>
            <div className="hd-word">FIT<span>COACH</span></div>
          </div>
          <div className="pulse-badge">
            <div className="pulse-dot" />
            <span className="pulse-txt">ACTIVO</span>
          </div>
        </div>
        {tab === "h" && <Dash />}
        {tab === "t" && <Train />}
        ...
        <div className="bn">
          {NAV.map(n => (
            <button key={n.id} className={`ni ${tab === n.id ? "on2" : ""}`} onClick={() => changeTab(n.id)}>
